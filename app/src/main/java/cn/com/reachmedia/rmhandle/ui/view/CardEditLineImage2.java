...
public class CardEditLineImage2 extends FrameLayout {
    ...
    ProportionImageView ivPestPhoto1;
    @Bind(R.id.iv_pest_photo_2)
    ProportionImageView ivPestPhoto2;
        ...
        if(!fragment.insertOrUpdate){//有本地未提交数据
            ...
            if(commBean!=null){
                if(!StringUtils.isEmpty(commBean.getCommunitySpace1())){
                    resultDatas1 = new PictureBean();
                    resultDatas1.setMainPath(commBean.getCommunitySpace1());
                    resultDatas1.setFileId(commBean.getCommunitySpaceId1());
                    resultDatas1.setType(PictureBean.PictureType.TYPE_1);
                }
                if(!StringUtils.isEmpty(commBean.getCommunitySpace2())){
                    resultDatas2 = new PictureBean();
    ...
    public void refreshAllImage(){
        if(resultDatas1!=null){
            resultDatas1.displayImage(ivPestPhoto1);
        }
        if(resultDatas2!=null){
            resultDatas2.displayImage(ivPestPhoto2);
        }
        setAddPhotosClickEvent(1, 1);
    }

    public void setAddPhotosClickEvent(final int index1,final int index2) {
        ivPestPhoto1.setOnClickListener(new OnClickListener() {
            @Override
        ...
        ivPestPhoto2.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if(resultDatas2==null){
                    takePhoto2(index2);
    ...
    public boolean updateAddPhotosClickState(Activity activity, Bundle savedInstanceState) {
        ...
        pickManger1 = new PhotoPickManger("cardedit3", activity, savedInstanceState, new PhotoPickManger.OnPhotoPickFinsh() {
            @Override
            public void onPhotoPick(List<File> list) {
                for (File file : list) {
                    String fileId = ImageUtils.getGatePicId(fragment.pointListModel.getCommunityid(),fragment.userId,3);
        ...
        pickManger2 = new PhotoPickManger("cardedit4", activity, savedInstanceState, new PhotoPickManger.OnPhotoPickFinsh() {
            @Override
            public void onPhotoPick(List<File> list) {
                for (File file : list) {
                    String fileId = ImageUtils.getGatePicId(fragment.pointListModel.getCommunityid(),fragment.userId,4);
                    String filePath = ImageUtils.getPointPicPath(fileId, LineImageLayout.photo_path);
                    PictureBean tempBean = new PictureBean(file, PictureBean.PictureType.TYPE_4,fileId,filePath);
    ...
    public CommDoorPicBean getDooBean(CommDoorPicBean bean){
        if(resultDatas1!=null && resultDatas1.getType()==PictureBean.PictureType.TYPE_4) {
            bean.setCommunitySpace1(resultDatas1.getMainPath());
            bean.setCommunitySpaceId1(resultDatas1.getFileId());
        }
        if(resultDatas2!=null && resultDatas2.getType()==PictureBean.PictureType.TYPE_4) {
            bean.setCommunitySpace2(resultDatas2.getMainPath());
